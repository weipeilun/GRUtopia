...
class One2OneQueue(AbstractQueue):
    ...
    def __init__(self, element_shape, element_dtype, shm=None, trigger_queue=None, signal_queue=None, signal_to_send=None, max_queue_size=100, ignore_array=False, put_timeout=0.001):
    ...
    def put(self, data):
            ...
            while True:
                try:
                    if interrupt.interrupt_callback():
                        break
                    signal = self.signal_queue.get(block=True, timeout=self.put_timeout)
                    break
                except Empty:
        ...
        try:
            self.trigger_queue.put(trigger, block=True, timeout=self.put_timeout)
        except Full:
            pass
    ...
    def get(self, block=True, timeout=None, ignore_array=False):
        ...
        if self.ignore_array:
            ...
            # 本次array取值忽略
            try:
                self.signal_queue.put(self.signal_to_send, block=True, timeout=self.put_timeout)
            except Full:
                pass
...
class One2ManyQueue(AbstractQueue):
    ...
    def __init__(self, element_shape, element_dtype, n_consumers, max_queue_size=100, broadcast=False, ignore_array=False, verbose=0, put_timeout=0.001):
        assert isinstance(element_shape, (tuple, list)), 'element_shape must be a list or tuple'
        ...
        for i in range(self.n_consumers):
            try:
                self.signal_queue.put(i, block=True, timeout=self.put_timeout)
            except Full:
                pass
    ...
    def put(self, data, broadcast_to=None):
        if self.ignore_array:
            trigger = data
        else:
            assert isinstance(data, (list, tuple)) and len(data) == 2, 'Data should contain and only contain trigger and array'
        ...
        if self.broadcast:
            ...
            if broadcast_to is None:
                for trigger_queue in self.trigger_queue_list:
                    try:
                        trigger_queue.put(trigger, block=True, timeout=self.put_timeout)
                    except Full:
                        pass
            else:
                for i in broadcast_to:
                    try:
                        self.trigger_queue_list[i].put(trigger, block=True, timeout=self.put_timeout)
                    except Full:
                        pass
            ...
            while True:
                try:
                    if interrupt.interrupt_callback():
                        break
                    idx = self.signal_queue.get(block=True, timeout=self.put_timeout)

                    if not self.ignore_array:
                        np.copyto(self.shm_np_list[idx], arr)

                    try:
                        self.trigger_queue_list[idx].put(trigger, block=True, timeout=self.put_timeout)
                    except Full:
                        pass
                    break
                except Empty:
